...
Suepercharge — autonomous legal lead-gen MVP. Three agents (ingest, creative, campaign) detect class-action press releases on PR Newswire, generate Meta ad creative, deploy campaigns, and capture leads. Approved plan lives at `~/.Codex/plans/peaceful-weaving-sundae.md`.
...
- `make parse URL=...` — single-URL Codex dry-run (no DB, no AWS)
...
- `agents/ingest.py` — cron 15m. Pulls PR Newswire RSS, parses cases with Codex (citation-enforced, fails closed), writes `Case` + `ICP`.
- `agents/creative.py` — cron 5m. Picks pending cases, generates copy (Codex) + image (Ideogram) + video (Arcads async, polled next tick), applies ffmpeg AI-disclosure watermark, posts to Slack for 👍/👎 approval.
...
`models.py` has the 5-table schema + Pydantic LLM output targets. `compliance.py` has the UPL/FTC phrase blocklist, the versioned TCPA consent text registry, and the AI-disclosure string. `clients/anthropic_client.py` wraps Codex with enforced Pydantic structured output via tool-use.
...
- Codex structured output via `clients.anthropic_client.structured(response_model=...)`. Don't call the SDK directly from agents.
